<?php
/**
 * page-testimonials.php — Sail Renovate
 * Auto-applied to the page with slug "testimonials" via WP template hierarchy.
 *
 * DO NOT add a "Template Name:" header — that converts this to an opt-in named
 * template and prevents WordPress from auto-applying it by slug.
 *
 * IMPORTANT: if the page body (Gutenberg editor) still contains hardcoded
 * testimonial content, clear it in WP Admin → Pages → Testimonials to prevent
 * duplicate output. The authoritative testimonials come from the CPT loop below.
 *
...
<main>
		...
		<div class="grid-3">
			...
			if ( $testi_q->have_posts() ) :
				while ( $testi_q->have_posts() ) : $testi_q->the_post();
					// Optional CPT ACF fields — absent until testimonial_rating /
			...
			?>
				<article class="about-testimonial fade-in">
					<div class="about-testimonial__stars" aria-label="<?php echo esc_attr( $t_stars ); ?> <?php esc_attr_e( 'out of 5 stars', 'sail-renovate' ); ?>">
						<?php echo str_repeat( '&#9733;', $t_stars ); ?>
					</div>
					<blockquote class="about-testimonial__quote">
						<?php the_content(); ?>
					</blockquote>
					<p class="about-testimonial__author"><?php echo esc_html( get_the_title() ); ?></p>
					<?php if ( $t_role ) : ?>
					<p class="about-testimonial__role"><?php echo esc_html( $t_role ); ?></p>
					<?php endif; ?>
				</article>
			<?php
				endwhile;
			...
			?>
				<p class="testimonials-empty"><?php esc_html_e( 'Client testimonials coming soon.', 'sail-renovate' ); ?></p>
			<?php endif; ?>
		</div>
	</section>
	...
	<section class="page-section--alt">
		<div class="inner" style="display: flex; flex-direction: column; align-items: center; text-align: center;">
			...
			<p style="color: var(--text-muted); font-size: 1.05rem; line-height: 1.7; max-width: 520px; margin: 1rem auto 0;"><?php echo esc_html( $cta_t ); ?></p>
			<?php endif; ?>
			<?php $btn_l = sail_field( 'testimonials_cta_button_label' ); if ( $btn_l ) : ?>
			<a class="btn btn--primary" href="<?php echo esc_url( sail_field( 'testimonials_cta_button_url', home_url( '/contact/' ) ) ); ?>" style="margin-top: 2rem;">
